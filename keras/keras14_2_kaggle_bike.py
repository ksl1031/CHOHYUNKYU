...
x = train_csv.drop(['count', 'casual', 'registered'], axis = 1) # count 컬럼을 제외한 나머지 데이터를 변수 x에 저장한다. drop : 지정한 컬럼을 제거한 결과를 반환한다. axis : 열방향으로 제거
# print(x)
...
#2 모델 구성
model = Sequential()
model.add(Dense(150, input_dim = 8))
...
model.fit(x_train, y_train, epochs = 700, batch_size = 800, verbose = 1)
...
def RMSE(y_test,y_predict): # 함수를 정의할때 사용 ():안에 입력값을 받아서 
    return np.sqrt(mean_squared_error(y_test,y_predict)) # RMSE 함수 정의 후에 np.sqrt에 반환
...
submission.to_csv(path_save + 'submit_0307_1216.csv')